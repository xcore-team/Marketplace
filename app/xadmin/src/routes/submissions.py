...
def submissions_router(db: Any, events: Any = None) -> APIRouter:
    ...
    async def list_submissions(
        ...
        source: Optional[str] = Query(None, description="upload | github"),
        ...
        async with db.session() as session:
            ...
            items = [
                SubmissionAdminOut(
                    id=r.id, developer_id=r.developer_id, developer_email=r.developer_email,
                    plugin_name=r.plugin_name, plugin_version=r.plugin_version,
                    status=r.status, source=r.source, anomaly_score=r.anomaly_score,
    ...
    async def set_status(
        ...
        async with db.session() as session:
            sub_row = await session.execute(
                sql_text("SELECT id, developer_id, plugin_name, plugin_version FROM market_submissions WHERE id = :id"),
                {"id": submission_id},
            )
            ...
            await session.execute(
                sql_text("UPDATE market_submissions SET status = :status WHERE id = :id"),
