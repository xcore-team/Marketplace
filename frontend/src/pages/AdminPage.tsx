...
type AdminTab = 'stats' | 'users' | 'plugins' | 'submissions' | 'categories' | 'system' | 'deployments' | 'services'
...
export default function AdminPage() {
  ...
  const { data: deploysData, isLoading: deploysLoading } = useQuery({
    queryKey: ['admin-deployments', deployStatusFilter],
    queryFn: () => adminApi.deployments({ status: deployStatusFilter || undefined, limit: 50 }),
    enabled: tab === 'deployments',
  })
  const adminDeploys: any[] = (deploysData as any)?.items ?? []

  const purgeDeploysMutation = useMutation({
    mutationFn: () => adminApi.purgeDeployments({ keep_per_bucket: 20, max_age_days: 90 }),
    onSuccess: (res: any) => {
      toast(`${res?.deleted ?? 0} déploiement(s) purgé(s).`, 'success')
  ...
  // ── Services (xservices) ──────────────────────────────────────────────────
  const { data: adminServicesData, isLoading: adminServicesLoading } = useQuery({
    queryKey: ['admin-services'],
    queryFn: () => adminApi.services({ limit: 50 }),
    enabled: tab === 'services',
  })
  const adminServices: any[] = Array.isArray(adminServicesData) ? adminServicesData : []
  ...
  const [serviceSubsFilter, setServiceSubsFilter] = useState<string>('all')
  const { data: serviceSubsData, isLoading: serviceSubsLoading } = useQuery({
    queryKey: ['admin-service-submissions', serviceSubsFilter],
    queryFn: () => adminApi.serviceSubmissions(serviceSubsFilter !== 'all' ? { status: serviceSubsFilter } : {}),
    enabled: tab === 'services',
  })
  const adminServiceSubs: any[] = Array.isArray(serviceSubsData) ? serviceSubsData : []

  const approveServiceSubMutation = useMutation({
    mutationFn: (id: string) => adminApi.approveServiceSubmission(id),
    onSuccess: () => {
      toast('Soumission approuvée.', 'success')
      queryClient.invalidateQueries({ queryKey: ['admin-service-submissions'] })
    },
    onError: (e: Error) => toast(e.message, 'error'),
  })

  const rejectServiceSubMutation = useMutation({
    mutationFn: (id: string) => adminApi.rejectServiceSubmission(id),
    onSuccess: () => {
      toast('Soumission rejetée.', 'info')
      queryClient.invalidateQueries({ queryKey: ['admin-service-submissions'] })
    },
    onError: (e: Error) => toast(e.message, 'error'),
  })
  ...
  const tabs: { id: AdminTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'deployments', label: 'Déploiements', icon: <Rocket size={16} /> },
  ...
  return (
    <div className="page">
      <div className="dash-header">
        <div className="dash-header__inner">
          ...
          <p className="dash-header__sub">
            Vue d'ensemble, utilisateurs, contenus et état du système.
          </p>
      ...
      <div className="section">
        ...
        {/* ── Services (xservices) ── */}
        {tab === 'services' && (
          <div style={{ animation: 'fadeUp 0.3s ease' }} className="flex-col gap-10">
            <div>
              <h3 className="font-bold mb-4 flex items-center gap-2">
                <Server size={16} style={{ color: 'var(--acc)' }} /> Services publiés
              </h3>
              {adminServicesLoading ? (
                <div className="flex items-center gap-2 text-muted"><div className="spinner" /> Chargement…</div>
              ) : adminServices.length === 0 ? (
                <div className="empty"><div className="empty__title">Aucun service</div></div>
              ) : (
                <div className="table-wrap">
                  <table className="table">
                    <thead><tr><th>Nom</th><th>Publication</th><th>Note</th><th>Installations</th><th>Actions</th></tr></thead>
                    <tbody>
                      {adminServices.map((s: any) => (
                        <tr key={s.id}>
                          <td>
                            <div className="font-bold text-sm">{s.name}</div>
                            <div className="text-xs font-mono text-faint">{s.slug}</div>
                          </td>
                          <td>{s.is_published ? <span className="badge badge-success">Publié</span> : <span className="badge badge-ghost">Brouillon</span>}</td>
                          <td><span className="font-mono text-sm">{s.avg_rating?.toFixed(1) ?? '—'}</span></td>
                          <td><span className="font-mono text-sm">{s.install_count ?? 0}</span></td>
                          <td>
                            <button
                              className="btn btn-ghost btn-sm btn-icon"
                              style={{ color: 'var(--danger)' }}
                              ...
                              onClick={() => {
                                if (confirm(`Supprimer définitivement le service "${s.name}" ?`)) deleteServiceMutation.mutate(s.id)
                              }}
                            >
                              <Trash2 size={13} />
                            </button>
            ...
            <div>
              <div className="flex items-center gap-2 mb-6" style={{ flexWrap: 'wrap' }}>
                {['all', 'pending', 'processing', 'manual_review', 'approved', 'rejected', 'failed'].map(s => (
                  <button
                    key={s}
                    className={`btn btn-sm ${serviceSubsFilter === s ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ fontSize: 11 }}
                    onClick={() => setServiceSubsFilter(s)}
                  >
                    {s === 'all' ? 'Toutes' : STATUS_LABEL[s] ?? s}
                  </button>
                ))}
              ...
              {serviceSubsLoading ? (
                <div className="flex items-center gap-2 text-muted"><div className="spinner" /> Chargement…</div>
              ) : adminServiceSubs.length === 0 ? (
                <div className="empty"><div className="empty__title">Aucune soumission de service</div></div>
              ) : (
                <div className="table-wrap">
                  <table className="table">
                    <thead><tr><th>Service</th><th>Version</th><th>Statut</th><th>Score</th><th>Actions</th></tr></thead>
                    <tbody>
                      {adminServiceSubs.map((s: any) => (
                        <tr key={s.id}>
                          <td className="font-bold text-sm">{s.service_name}</td>
                          <td><span className="font-mono text-sm">v{s.service_version}</span></td>
                          <td>
                            <span className="badge" style={{
                              background: `${STATUS_COLOR[s.status] ?? 'var(--surface2)'}22`,
                              color: STATUS_COLOR[s.status] ?? 'var(--text2)',
                          ...
                          <td>{s.anomaly_score != null ? <span className="font-mono text-sm">{s.anomaly_score}</span> : <span className="text-faint">—</span>}</td>
                          <td>
                            {(s.status === 'manual_review' || s.status === 'pending') && (
                              <div className="flex items-center gap-1">
                                <button className="btn btn-ghost btn-sm" style={{ color: 'var(--success)', fontSize: 11, padding: '3px 8px' }}
                                  disabled={approveServiceSubMutation.isPending}
                                  onClick={() => approveServiceSubMutation.mutate(s.id)}>
                                  <CheckCircle size={12} /> Approuver
                                </button>
                                <button className="btn btn-ghost btn-sm" style={{ color: 'var(--danger)', fontSize: 11, padding: '3px 8px' }}
                                  disabled={rejectServiceSubMutation.isPending}
                                  onClick={() => rejectServiceSubMutation.mutate(s.id)}>
                                  <XCircle size={12} /> Rejeter
                                </button>
                              </div>
                            )}
            ...
            <div className="flex items-center justify-between mb-6" style={{ flexWrap: 'wrap', gap: 12 }}>
              <div className="flex items-center gap-2" style={{ flexWrap: 'wrap' }}>
                {['', 'pending', 'running', 'succeeded', 'failed'].map(s => (
                  <button
                    key={s}
                    className={`btn btn-sm ${deployStatusFilter === s ? 'btn-primary' : 'btn-ghost'}`}
                    style={{ fontSize: 11 }}
                    onClick={() => setDeployStatusFilter(s)}
                  >
                    {s === '' ? 'Tous' : s}
                  </button>
                ))}
              ...
              <button
                ...
                disabled={purgeDeploysMutation.isPending}
                onClick={() => {
                  if (confirm('Purger les anciens déploiements (garder 20 par groupe, max 90 jours) ?')) purgeDeploysMutation.mutate()
                }}
              >
                {purgeDeploysMutation.isPending ? 'Purge…' : 'Purger les anciens'}
              </button>
            </div>
            {deploysLoading ? (
              <div className="flex items-center gap-2 text-muted"><div className="spinner" /> Chargement…</div>
            ) : adminDeploys.length === 0 ? (
              <div className="empty"><div className="empty__title">Aucun déploiement</div></div>
            ) : (
              <div className="table-wrap">
                <table className="table">
                  <thead><tr><th>Slug</th><th>Type</th><th>Version</th><th>Développeur</th><th>Statut</th><th>Hôte</th><th>Date</th></tr></thead>
                  <tbody>
                    {adminDeploys.map((d: any) => (
                      <tr key={d.id}>
                        <td className="font-bold text-sm">{d.slug}</td>
                        <td><span className="badge badge-ghost">{d.kind}</span></td>
                        <td><span className="font-mono text-sm">v{d.version}</span></td>
                        <td><span className="text-xs text-muted">{d.deployer_email ?? d.deployer_id?.slice(0, 8)}</span></td>
                        <td>
                          <span className="badge" style={{
                            background: `${STATUS_COLOR[d.status] ?? 'var(--surface2)'}22`,
                            color: STATUS_COLOR[d.status] ?? 'var(--text2)',
                            border: `1px solid ${STATUS_COLOR[d.status] ?? 'var(--border)'}44`,
                          }}>
                            {d.status}
                          </span>
                        </td>
                        <td><span className="text-xs font-mono text-faint">{d.host_id}</span></td>
                        <td><span className="text-xs text-muted">{new Date(d.created_at).toLocaleDateString('fr', { day: 'numeric', month: 'short' })}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
