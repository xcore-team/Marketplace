...
import type { Plugin, Submission, SubmissionReport, SubmissionStatus, PipelineGate, Finding } from '../../types'
...
function GateRow({ gate }: { gate: PipelineGate }) {
  const hasF = gate.findings?.length > 0
  const [open, setOpen] = useState(gate.status !== 'passed' && hasF)

  return (
    <div className="list-row" style={{ flexDirection: 'column', alignItems: 'stretch', cursor: hasF ? 'pointer' : 'default' }} onClick={() => hasF && setOpen(o => !o)}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <StatusIcon status={gate.status} size={14} />
        <span style={{ flex: 1, fontSize: 13, fontWeight: 500 }}>{gate.gate}</span>
        {gate.anomaly_score > 0 && (
          <span style={{ fontSize: 11, fontFamily: 'var(--f-mono)', color: gate.anomaly_score >= 40 ? 'var(--danger)' : 'var(--warning)' }}>+{gate.anomaly_score}</span>
        )}
        {gate.duration_seconds != null && <span style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--f-mono)' }}>{(gate.duration_seconds * 1000).toFixed(0)}ms</span>}
        {hasF && <div style={{ color: 'var(--text3)' }}>{open ? <ChevronUp size={13} /> : <ChevronDown size={13} />}</div>}
      </div>
      {open && hasF && (
        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {gate.findings.map((f: Finding, i: number) => {
            const s = SEV[f.severity] ?? SEV.info
            return (
              <div key={i} style={{ borderLeft: `3px solid ${s.color}`, paddingLeft: 10 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: f.file ? 4 : 0 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 'var(--r-full)', background: s.bg, color: s.color, flexShrink: 0 }}>{s.label}</span>
                  <span style={{ fontSize: 12, lineHeight: 1.5, color: 'var(--text2)' }}>{f.message}</span>
                </div>
                {f.file && <code style={{ fontSize: 11, color: 'var(--acc)', fontFamily: 'var(--f-mono)' }}>{f.file}{f.line != null ? `:${f.line}` : ''}</code>}
              </div>
            )
          })}
        </div>
...
function PipelineReport({ submissionId }: { submissionId: string }) {
  const { data, isLoading } = useQuery({ queryKey: ['sub-report', submissionId], queryFn: () => subsApi.report(submissionId), retry: false })

  if (isLoading) return <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text3)', fontSize: 13, padding: '8px 0' }}><Loader2 size={14} className="spin" /> Chargement du rapport…</div>
  if (!data) return <div style={{ fontSize: 12, color: 'var(--text3)', padding: '6px 0' }}>Rapport non disponible.</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', padding: '12px 16px' }}>
        {report.status && <StatusIcon status={report.status} size={15} />}
        <span style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>
          {report.recommendation ?? (
            report.status === 'approved' ? 'Approuvé automatiquement'
              : report.status === 'rejected' ? 'Rejeté automatiquement'
                : report.status === 'manual_review' ? 'Révision manuelle'
                  : 'Pipeline terminé'
          )}
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {report.summary && (
            <div style={{ display: 'flex', gap: 6 }}>
              {report.summary.critical > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: '#ff4444' }}>{report.summary.critical}C</span>}
              {report.summary.high > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: '#ff8c00' }}>{report.summary.high}H</span>}
              {report.summary.medium > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--warning)' }}>{report.summary.medium}M</span>}
            </div>
          )}
          ...
          <span style={{ fontSize: 12, fontFamily: 'var(--f-mono)', color: scoreColor, fontWeight: 700 }}>{score}</span>
        </div>
      </div>

      {gates.length > 0 && <div className="list">{gates.map(g => <GateRow key={g.gate} gate={g} />)}</div>}

      {report.merkle_root && (
        <div style={{ padding: '6px 10px', background: 'var(--surface2)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)' }}>
          <span style={{ fontSize: 11, fontFamily: 'var(--f-mono)', color: 'var(--text3)' }}>merkle: {report.merkle_root}</span>
        </div>
      )}
    </div>
  )
                ...
                {submissionList.map((sub) => {
                  ...
                  return (
                    <div key={sub.id} className="panel" style={{ background: isOpen ? 'var(--surface2)' : 'var(--surface)' }}>
                      ...
                      {isOpen && <div style={{ borderTop: '1px solid var(--border)', padding: 16 }}><PipelineReport submissionId={sub.id} /></div>}
                    </div>
                  )
                })}
              </div>
