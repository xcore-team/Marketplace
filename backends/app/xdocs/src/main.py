"""xdocs — Documentation des plugins publiés, récupérée en direct depuis GitHub.

Après validation d'un plugin soumis depuis un repo GitHub, le worker Celery
récupère 3 fichiers depuis le repo au tag Git publié (GitHubService.fetch_docs) :
  - README.md                    → documentation principale
  - integration.yaml             → configuration d'intégration xcore
...
class Plugin(AutoDispatchMixin, TrustedBase):
    async def on_load(self) -> None:
        ...
        self.app.include_router(docs_router(db, ctx=self.call_plugin))
