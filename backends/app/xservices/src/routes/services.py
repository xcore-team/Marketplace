...
async def _api_key_viewer_id(x_api_key: Optional[str], ctx: Any) -> Optional[str]:
    ...
    xdevkey (xcore-agent watch-sources, xcli sans session navigateur) était
    toujours traité comme anonyme et se voyait refuser toute extension
    visibility="private" avec un 404, même en possédant une clé valide qui
    y donne accès. Ne lève jamais — dégrade vers "aucune identité API key"
    si la clé est absente/invalide, exactement comme l'absence de JWT."""
    if not x_api_key:
        return None
