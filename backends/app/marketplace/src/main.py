...
class Plugin(IPCCommands, AutoDispatchMixin, TrustedBase):
    ...
    async def on_load(self) -> None:
        ...
        self.app.include_router(github_router(db, events, secret_key, limits, ctx=self.call_plugin))
        self.app.include_router(webhooks_router(db))
        self.app.include_router(install_router(db, devkeys_master, ctx=self.call_plugin))
