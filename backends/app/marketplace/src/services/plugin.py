...
class PluginService:
    ...
    def _visibility_clause(viewer_id: Optional[str], viewer_tenant_ids: Optional[set]):
        """(public) OU (privé ET (propriétaire OU membre de l'équipe propriétaire)).

        `viewer_tenant_ids` est aujourd'hui peuplé avec au plus l'équipe
        active du viewer (claim `tenant_id` du JWT) — pas "toutes les équipes
        dont il a jamais été membre" (ce que faisait xorgs avec un set
        multi-org). Reste un set pour pouvoir élargir plus tard sans
        retoucher cette clause, si on ajoute une vraie liste d'appartenances."""
        clause = Plugin.visibility == "public"
        if viewer_id:
            clause = clause | (Plugin.developer_id == viewer_id)
        if viewer_tenant_ids:
            clause = clause | (Plugin.tenant_id.in_(viewer_tenant_ids))
        return clause
    ...
    async def can_view(
        ...
        if plugin.visibility != "private":
            return True
        if viewer_id and viewer_id == plugin.developer_id:
            return True
        if plugin.tenant_id and viewer_tenant_ids and plugin.tenant_id in viewer_tenant_ids:
            return True
        return False
