...
async def _api_key_viewer_id(x_api_key: Optional[str], ctx: Any) -> Optional[str]:
    ...
    (JWT), donc tout appelant xdevkey (xcore-agent, xcli sans session
    navigateur) était systématiquement traité comme anonyme et se voyait
    refuser tout plugin visibility="private" avec un 404, même en possédant
    une clé valide qui y donne accès (MarketplaceClient.get_latest_version
    n'envoie ni Bearer ni X-API-Key aujourd'hui côté agent — moitié du
    correctif ; ce endpoint doit d'abord être capable de le reconnaître).
