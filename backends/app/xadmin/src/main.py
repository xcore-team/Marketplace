...
Centralise la gestion de toutes les ressources de la plateforme :
  ...
  - Soumissions (forcer un statut) — inclut les soumissions déclenchées par CI
    (source="ci", voir POST /github/repos/{owner}/{repo}/tags/{tag}/recompute
    côté marketplace/xservices), filtrables via GET /submissions?source=ci
  - Catégories (CRUD)
  - Équipes : gérées par app/auth (Tenant/TenantMember/RBAC), pas ici — le
    plugin xorgs qui doublonnait cette notion (Organization) a été retiré,
    voir la réconciliation xorgs -> tenant (models/plugin.py, services/service.py).
  - Déploiements (xdeployments) — rapports xcore-agent, purge manuelle
  - Audit logs
  - Stats globales
