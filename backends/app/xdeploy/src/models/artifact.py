...
class XDeployArtifact(Base):
    ...
    build + publish). Le ciphertext lui-même vit dans le stockage blob
    partagé (voir services/artifact.py + extensions/xstorage — jamais en
    base, un artefact peut faire plusieurs Mo, et rien dans cet écosystème ne
    stocke de blob binaire en colonne DB) ; cette table ne garde que les
    métadonnées nécessaires pour le retrouver, le resservir et déverrouiller
    son DEK.
    """
    ...
    # Requis par ext.storage pour read()/delete() — la clé physique est
    # "{namespace}/{stored_name}" et l'extension ne peut pas se reconstruire
    # depuis l'id seul (voir extensions/xstorage/uploader.py).
